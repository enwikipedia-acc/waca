<tr>
  ...
  <td>{if $r->hasComments() == true}<span class="label label-info visible-desktop">Comment</span>{/if}</td>
	<td>{if $currentUser->isAdmin() || $currentUser->isCheckUser() == true}<a href="mailto:{$r->getEmail()}" target="_blank">{$r->getEmail()}</a>&nbsp;<span class="badge{if $r->numberOfEmailRequests() > 0} badge-important{/if}">{$r->numberOfEmailRequests()}</span><span class="hidden-desktop"><br /><a href="https://en.wikipedia.org/wiki/User_talk:{$r->getTrustedIp()}" target="_blank">{$r->getTrustedIp()}</a>&nbsp;<span class="badge {if $r->numberOfIpRequests() > 0} badge-important{/if}">{$r->numberOfIpRequests()}</span>{/if}
		<span class="visible-phone"><br /><a href="https://en.wikipedia.org/wiki/User:{$r->getName()}" target="_blank">{$r->getName()}</a></span></span></td>
	<td>{if $currentUser->isAdmin() || $currentUser->isCheckUser() == true}<span class="visible-desktop"><a href="https://en.wikipedia.org/wiki/User_talk:{$r->getTrustedIp()}" target="_blank">{$r->getTrustedIp()}</a>&nbsp;<span class="badge {if $r->numberOfIpRequests() > 0} badge-important{/if}">{$r->numberOfIpRequests()}</span></span>{/if}</td>
