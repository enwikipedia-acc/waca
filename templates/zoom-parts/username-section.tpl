    ...
    <table class="table table-condensed table-striped">
      {foreach $spoofs as $spoof}
        ...
        <tr>
          ...
          <td>
            <a id="SpoofContribs-{$spoof@iteration}" class="btn btn-small" target="_blank" href="https://en.wikipedia.org/wiki/Special:Contributions/{$spoof|escape:'url'}" OnClick="$('#SpoofContribs-{$spoof@iteration}').css('background-image', 'linear-gradient(to bottom, rgb(0, 255, 0), rgb(0, 230, 0))');">Contributions</a>
            <a id="SpoofLogs-{$spoof@iteration}" class="btn btn-small" target="_blank" href="https://en.wikipedia.org/w/index.php?title=Special%3ALog&amp;type=&amp;user=&amp;page=User%3A{$spoof|escape:'url'}&amp;year=&amp;month=-1&amp;tagfilter=&amp;hide_patrol_log=1&amp;hide_review_log=1&amp;hide_thanks_log=1" OnClick="$('#SpoofLogs-{$spoof@iteration}').css('background-image', 'linear-gradient(to bottom, rgb(0, 255, 0), rgb(0, 230, 0))');">Logs</a>
            <a id="SpoofSUL-{$spoof@iteration}" class="btn btn-small" target="_blank" href="http://toolserver.org/~quentinv57/tools/sulinfo.php?showinactivity=1&amp;showblocks=1&amp;username={$spoof|escape:'url'}" OnClick="$('#SpoofSUL-{$spoof@iteration}').css('background-image', 'linear-gradient(to bottom, rgb(0, 255, 0), rgb(0, 230, 0))');">SUL</a>
            <a id="SpoofCentralAuth-{$spoof@iteration}" class="btn btn-small" target="_blank" href="https://en.wikipedia.org/wiki/Special:CentralAuth/{$spoof|escape:'url'}" OnClick="$('#SpoofCentralAuth-{$spoof@iteration}').css('background-image', 'linear-gradient(to bottom, rgb(0, 255, 0), rgb(0, 230, 0))');">Special:CentralAuth</a>
            <a id="SpoofPassReset-{$spoof@iteration}" class="btn btn-small" target="_blank" href="https://en.wikipedia.org/wiki/Special:PasswordReset?wpUsername={$spoof|escape:'url'}" OnClick="$('#SpoofPassReset-{$spoof@iteration}').css('background-image', 'linear-gradient(to bottom, rgb(0, 255, 0), rgb(0, 230, 0))');">Send Password reset</a>
            <a id="SpoofCount-{$spoof@iteration}" class="btn btn-small" target="_blank" href="https://tools.wmflabs.org/xtools/pcount/index.php?lang=en&amp;wiki=wikipedia&amp;name={$spoof|escape:'url'}" OnClick="$('#SpoofCount-{$spoof@iteration}').css('background-image', 'linear-gradient(to bottom, rgb(0, 255, 0), rgb(0, 230, 0))');">Count</a>
          </td>
        </tr>
      {/foreach}
    </table>
