...
CREATE PROCEDURE SCHEMA_UPGRADE_SCRIPT() BEGIN
    ...
    -- -------------------------------------------------------------------------
    DECLARE patchversion INT DEFAULT 12;
    -- -------------------------------------------------------------------------
    -- working variables
    ...
    ALTER TABLE `request` 
        CHANGE COLUMN `comment` `oldcomment` LONGTEXT NOT NULL;

    ALTER TABLE `request`
        ADD COLUMN `comment` VARCHAR(3000) NOT NULL AFTER `oldcomment`;
        
    UPDATE `request` 
        SET `comment` = SUBSTR(`oldcomment`, 1, 3000) 
        WHERE `oldcomment` <> '';

    ALTER TABLE `request`
        DROP COLUMN `oldcomment`;
