...
CREATE PROCEDURE SCHEMA_UPGRADE_SCRIPT() BEGIN
    ...
    -- -------------------------------------------------------------------------
    DECLARE patchversion INT DEFAULT 52;
    -- -------------------------------------------------------------------------
    -- working variables
    ...
    alter table ban
        drop constraint ban_check_action;

    alter table ban
        add constraint ban_check_action
            check (`action` in ('block', 'drop', 'drop-pre', 'defer', 'none'));

    alter table requestdata
        modify type varchar(20) not null comment 'The type of private data stored';

    alter table requestdata
        drop constraint requestdata_check_type;

    alter table requestdata
        add constraint requestdata_check_type
            check (`type` in
                   ('ipv4', 'ipv6', 'email', 'useragent', 'clienthint',
                    'confirm-ipv4', 'confirm-ipv6', 'confirm-useragent', 'confirm-clienthint'
                   ));
