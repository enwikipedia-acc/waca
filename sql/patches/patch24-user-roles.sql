...
CREATE PROCEDURE SCHEMA_UPGRADE_SCRIPT() BEGIN
    ...
    -- -------------------------------------------------------------------------
    DECLARE patchversion INT DEFAULT 24;
    -- -------------------------------------------------------------------------
    -- working variables
    ...
    (
        ...
        CONSTRAINT userrole_user_id_fk FOREIGN KEY (user) REFERENCES user (id)
    );

    CREATE UNIQUE INDEX userrole_user_role_uindex ON userrole (user, role);
