...
CREATE PROCEDURE SCHEMA_UPGRADE_SCRIPT() BEGIN
    ...
    -- -------------------------------------------------------------------------
    DECLARE patchversion INT DEFAULT 28;
    -- -------------------------------------------------------------------------
    -- working variables
    ...
    (
        ...
        enqueue TIMESTAMP DEFAULT current_timestamp NOT NULL,
        parameters BLOB DEFAULT NULL,
        error VARCHAR(255) DEFAULT NULL,
        acknowledged INT,
        parent INT DEFAULT NULL,
        CONSTRAINT jobqueue_user_id_fk FOREIGN KEY (user) REFERENCES user (id),
        CONSTRAINT jobqueue_request_id_fk FOREIGN KEY (request) REFERENCES request (id),
        CONSTRAINT jobqueue_emailtemplate_id_fk FOREIGN KEY (emailtemplate) REFERENCES emailtemplate (id),
        CONSTRAINT jobqueue_parent_id_fk FOREIGN KEY (parent) REFERENCES jobqueue (id),
        INDEX jobqueue_status_enqueue_index (status, enqueue)
    ) ENGINE=InnoDB COLLATE=utf8mb4_unicode_ci;

    ALTER TABLE user ADD COLUMN creationmode INT(11) NOT NULL DEFAULT 0;
