...
function addCommand($command, $callback, $forked = false) {
	...
	$commands[strtolower($command)] = array (
		$callback,
		$forked
	);
}
...
function parseIrc($line) {
	...
	if (strtolower($explode1[0]) == 'ping') {
		...
		if (strtolower($explode1[1]) == 'privmsg') {
			$return['type'] = 'privmsg';
			$return['n!u@h'] = (($explode1[0] { 0 }
			== ':') ? substr($explode1[0], 1) : $explode1[0]);
			...
			$return['message'] = (($explode1[3] { 0 }
			== ':') ? substr($explode1[3], 1) : $explode1[3]);
...
function commandCount($parsed) {
	...
	$isUser = mysql_fetch_assoc(myq('SELECT COUNT(*) AS `count` FROM `acc_user` WHERE `user_name` = \'' . sanitize($username) . '\'')) or die('MySQL Error: ' . mysql_error() . "\n");

	$isUser = (($isUser['count'] == 0) ? false : true);

	if ($isUser) {
		$count = mysql_fetch_assoc(myq('SELECT COUNT(*) AS `count` FROM `acc_log` WHERE `log_action` = \'Closed 1\' AND `log_user` = \'' . sanitize($username) . '\'')) or die('MySQL Error: ' . mysql_error() . "\n");

		$count = $count['count'];

		$user = mysql_fetch_assoc(myq('SELECT * FROM `acc_user` WHERE `user_name` = \'' . sanitize($username) . '\'')) or die('MySQL Error: ' . mysql_error() . "\n");

		if ($user['user_level'] == 'Admin') {
			$sus = mysql_fetch_assoc(myq('SELECT COUNT(*) AS `count` FROM `acc_log` WHERE `log_user` = \'' . sanitize($username) . '\' AND `log_action` = \'Suspended\'')) or die('MySQL Error: ' . mysql_error() . "\n");
			$sus = $sus['count'];

			$pro = mysql_fetch_assoc(myq('SELECT COUNT(*) AS `count` FROM `acc_log` WHERE `log_user` = \'' . sanitize($username) . '\' AND `log_action` = \'Promoted\'')) or die('MySQL Error: ' . mysql_error() . "\n");
			$pro = $pro['count'];

			$app = mysql_fetch_assoc(myq('SELECT COUNT(*) AS `count` FROM `acc_log` WHERE `log_user` = \'' . sanitize($username) . '\' AND `log_action` = \'Approved\'')) or die('MySQL Error: ' . mysql_error() . "\n");
			$app = $app['count'];

			$adminInfo = 'Suspended: ' . $sus . ', Promoted: ' . $pro . ', Approved: ' . $app;
		}

		$today = mysql_fetch_assoc(myq('SELECT COUNT(*) AS `count` FROM `acc_log` WHERE `log_time` LIKE \'' . sanitize(date('Y-m-d')) . '%\' AND `log_action` = \'Closed 1\' AND `log_user` = \'' . sanitize($username) . '\'')) or die('MySQL Error: ' . mysql_error() . "\n");
		$today = $today['count'];

		irc('PRIVMSG ' . $parsed['to'] . ' :' . $username . ' (' . $user['user_level'] . ') has closed ' . $count . ' requests as \'Done\', ' . (($today == 0) ? 'none' : $today) . ' of them today. ' . $adminInfo);
	} else {
...
function commandStatus($parsed) {
	$open = mysql_fetch_assoc(myq('SELECT COUNT(*) AS `count` FROM `acc_pend` WHERE `pend_status` = \'Open\'')) or die('MySQL Error: ' . mysql_error() . "\n");
	$open = $open['count'];

	$adminRequests = mysql_fetch_assoc(myq('SELECT COUNT(*) AS `count` FROM `acc_pend` WHERE `pend_status` = \'Admin\'')) or die('MySQL Error: ' . mysql_error() . "\n");
	$adminRequests = $adminRequests['count'];

	$bans = mysql_fetch_assoc(myq('SELECT COUNT(*) AS `count` FROM `acc_ban`')) or die('MySQL Error: ' . mysql_error() . "\n");
	$bans = $bans['count'];

	$admins = mysql_fetch_assoc(myq('SELECT COUNT(*) AS `count` FROM `acc_user` WHERE `user_level` = \'Admin\'')) or die('MySQL Error: ' . mysql_error() . "\n");
	$admins = $admins['count'];

	$users = mysql_fetch_assoc(myq('SELECT COUNT(*) AS `count` FROM `acc_user` WHERE `user_level` = \'User\'')) or die('MySQL Error: ' . mysql_error() . "\n");
	$users = $users['count'];

	$new = mysql_fetch_assoc(myq('SELECT COUNT(*) AS `count` FROM `acc_user` WHERE `user_level` = \'New\'')) or die('MySQL Error: ' . mysql_error() . "\n");
	$new = $new['count'];

	irc('PRIVMSG ' . $parsed['to'] . ' :' . 'Open requests: ' . $open . ', Admin requests: ' . $adminRequests . ', Banned: ' . $bans . ', Site users: ' . $users . ', Site admins: ' . $admins . ', Awaiting approval: ' . $new);
}
...
function commandStats($parsed) {
	...
	if ($isUser) {
		...
		irc('PRIVMSG ' . $parsed['to'] . ' :' . $username . ' (' . $user['user_level'] . ') was last active ' . (($user['user_lastactive'] == '0000-00-00 00:00:00') ? 'unknown' : $user['user_lastactive']) . '. He/she currently has automatic welcoming of users ' . (($user['user_welcome'] == 1) ? 'enabled' : 'disabled') . '. His/her onwiki username is [[User:' . $user['user_onwikiname'] . ']].');
