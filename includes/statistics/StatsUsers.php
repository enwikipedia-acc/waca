...
class StatsUsers extends StatisticsPage
{
	...
	function getUserDetail($userId)
	{
		...
		$query = "SELECT * FROM acc_log JOIN acc_user ON user_name = log_user JOIN acc_pend ON pend_id = log_pend WHERE user_id = " . $gid . " AND (log_action != 'Closed 1' AND log_action != 'Closed custom-y') AND log_action LIKE 'Closed %' AND log_action != 'Closed custom';";
