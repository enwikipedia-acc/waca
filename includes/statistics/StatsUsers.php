...
class StatsUsers extends StatisticsPage
{
	...
	function getUserDetail($userId)
	{
		...
		$qb = new QueryBrowser();
		$out .= $qb->executeQueryToTable('SELECT mail_desc AS "Close type", COUNT(*) AS Count FROM p_acc_live.acc_log l INNER JOIN closes ON `CONCAT("Closed ",mail_id)` = l.log_action WHERE l.log_user = "'.$row['user_name'].'" AND l.log_action LIKE "Closed%" GROUP BY l.log_action;');
		
		
		// List the requests this user has marked as 'created'
		$out.= "<h2>Users created</h2>\n";
		$query = "SELECT * FROM acc_log JOIN acc_pend ON pend_id = log_pend WHERE user_name = " . $siuser . " AND (log_action = 'Closed 1' OR log_action = 'Closed custom-y');";
		...
		$query = "SELECT * FROM acc_log JOIN acc_pend ON pend_id = log_pend WHERE user_name = " . $siuser . " AND (log_action != 'Closed 1' AND log_action != 'Closed custom-y') AND log_action LIKE 'Closed %' AND log_action != 'Closed custom';";
